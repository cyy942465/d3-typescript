...
import {computePowerDiagramIntegrated, polygonDirection} from './calculateWeight'
...
function weightedVoronoi() {
  /////// Inputs ///////
  var x = function (d: any) {
    return d.x;
  }; // accessor to the x value
  var y = function (d: any) {
    return d.y;
  }; // accessor to the y value
  var weight = function (d: any) {
    return d.weight;
  ...
  function _weightedVoronoi(data: any) {
    var formatedSites;

    //begin: map sites to the expected format of PowerDiagram
    formatedSites = data.map(function (d: any) {
  ...
  _weightedVoronoi.extent = function (_: any) {
    if (!arguments.length) {
      return extent;
    }
  ...
  _weightedVoronoi.size = function (_: any) {
    if (!arguments.length) {
      return size;
    }
